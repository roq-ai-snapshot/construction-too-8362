...
function AdminCreatePage() {
  ...
  const formik = useFormik<AdminInterface>({
    initialValues: {
      assigned_date: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="assigned_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Assigned Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.assigned_date ? new Date(formik.values?.assigned_date) : null}
              onChange={(value: Date) => formik.setFieldValue('assigned_date', value)}
            />
          </FormControl>
