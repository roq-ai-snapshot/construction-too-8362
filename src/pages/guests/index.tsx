...
export function GuestListPage(props: GuestListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getGuests({
        relations: ['user', 'rental_guest_rental_idTorental', 'rental_rental_guest_idToguest.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['phone_number.contains', 'address.contains', 'email.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('rental', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'rental_guest_rental_idTorental',
          header: 'Rental Guest Rental Id Torental',
          accessorKey: 'rental_guest_rental_idTorental',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/rentals/view/${record.rental_guest_rental_idTorental?.id}`}
            >
              {record.rental_guest_rental_idTorental?.start_date}
            </Link>
          ),
        }
      : null,
    hasAccess('rental', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'rental_rental_guest_idToguest',
          header: 'Rental Rental Guest Id Toguest',
          accessorKey: 'rental_rental_guest_idToguest',
          cell: ({ row: { original: record } }: any) => record?._count?.rental_rental_guest_idToguest,
        }
      : null,
