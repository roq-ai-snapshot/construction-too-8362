...
function GuestViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<GuestInterface>(
    ...
    () =>
      getGuestById(id, {
        relations: ['user', 'rental_guest_rental_idTorental'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('rental', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Rental Guest Rental Id Torental"
                    text={
                      <Link as={NextLink} href={`/rentals/view/${data?.rental_guest_rental_idTorental?.id}`}>
                        {data?.rental_guest_rental_idTorental?.start_date}
                      </Link>
                    }
                  />
                )}
