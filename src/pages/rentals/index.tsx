...
export function RentalListPage(props: RentalListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getRentals({
        relations: ['tool', 'guest_rental_guest_idToguest', 'guest_guest_rental_idTorental.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('guest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'guest_rental_guest_idToguest',
          header: 'Guest Rental Guest Id Toguest',
          accessorKey: 'guest_rental_guest_idToguest',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/guests/view/${record.guest_rental_guest_idToguest?.id}`}
            >
              {record.guest_rental_guest_idToguest?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('guest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'guest_guest_rental_idTorental',
          header: 'Guest Guest Rental Id Torental',
          accessorKey: 'guest_guest_rental_idTorental',
          cell: ({ row: { original: record } }: any) => record?._count?.guest_guest_rental_idTorental,
        }
      : null,
