...
function RentalViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RentalInterface>(
    ...
    () =>
      getRentalById(id, {
        relations: ['tool', 'guest_rental_guest_idToguest'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('guest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Guest Rental Guest Id Toguest"
                    text={
                      <Link as={NextLink} href={`/guests/view/${data?.guest_rental_guest_idToguest?.id}`}>
                        {data?.guest_rental_guest_idToguest?.email}
                      </Link>
                    }
                  />
                )}
